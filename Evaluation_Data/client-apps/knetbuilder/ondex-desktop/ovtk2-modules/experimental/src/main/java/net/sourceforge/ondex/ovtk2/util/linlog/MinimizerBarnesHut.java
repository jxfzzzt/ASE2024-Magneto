 ...
 * downloadable) article Andreas Noack: <a
 * href="http://jgaa.info/volume11.html">
...
public class MinimizerBarnesHut {
	...
	static class OctTree {
		 ...
		 * @param minPos
		 *            minimum coordinates of the cuboid
		 * @param maxPos
		 *            maximum coordinates of the cuboid
		 */
		public OctTree(Node node, double[] position, double[] minPos,
				double[] maxPos) {
			this.node = node;
		 ...
		 *            position of the graph node
		 * @param depth
		 *            depth of this tree node in the octtree
		 */
		public void addNode(Node newNode, double[] newPos, int depth) {
			...
			for (int d = 0; d < 3; d++) {
				position[d] = (weight * position[d] + newNode.weight
						* newPos[d])
		...
		protected void addNode2(Node newNode, double[] newPos, int depth) {
			if (depth == MAX_DEPTH) {
				if (children.length == childCount) {
					...
					System.arraycopy(oldChildren, 0, children, 0,
							oldChildren.length);
				}
				children[childCount++] = new OctTree(newNode, newPos, newPos,
						newPos);
				return;
			}
			...
			if (children[childIndex] == null) {
				...
				childCount++;
				children[childIndex] = new OctTree(newNode, newPos, newMinPos,
						newMaxPos);
			} else {
		...
		public void removeNode(Node oldNode, double[] oldPos, int depth) {
			...
			for (int d = 0; d < 3; d++) {
				position[d] = (weight * position[d] - oldNode.weight
						* oldPos[d])
	 ...
	 * 
	 * @param nodes
	 *            nodes with weights specifying their repulsion strengths. More
	 *            precisely, the repulsion between every pair of different nodes
	 *            is proportional to the product of their weights. It is
	 *            recommended to use so-called "edge-repulsion", which means to
	 *            set the weight of each node to the sum of the weights of its
	 *            attraction edges. Weights must not be negative.
	 * @param attrEdges
	 *            attraction edges. Omit edges with weight 0.0 (i.e. non-edges).
	 *            For unweighted graphs use weight 1.0 for all edges. Weights
	 *            must not be negative. Weights must be symmetric, i.e. the
	 *            weight from node <code>n1</code> to node <code>n2</code> must
	 *            be equal to the weight from node <code>n2</code> to node
	 *            <code>n1</code>.
	 * @param repuExponent
	 *            exponent of the distance in the repulsion energy. Exception:
	 *            The value 0.0 corresponds to logarithmic repulsion. Is 0.0 in
	 *            both the LinLog and the Fruchterman-Reingold energy model.
	 *            Negative values are permitted.
	 * @param attrExponent
	 *            exponent of the distance in the attraction energy. Is 1.0 in
	 *            the LinLog model (which is used for computing clusters, i.e.
	 *            dense subgraphs), and 3.0 in standard energy model of
	 *            Fruchterman and Reingold. Must be greater than
	 *            <code>repuExponent</code>.
	 * @param gravFactor
	 *            factor for the gravitation energy. Gravitation attracts each
	 *            node to the barycenter of all nodes, to prevent distances
	 *            between unconnected graph components from approaching
	 *            infinity. Typical values are 0.0 if the graph is guaranteed to
	 *            be connected, and positive values significantly smaller 1.0
	 *            (e.g. 0.05) otherwise.
	 */
	public MinimizerBarnesHut(final Collection<Node> nodes,
			final Collection<Edge> attrEdges, final double repuExponent,
			final double attrExponent, final double gravFactor) {
	 ...
	 *            attracting node
	 * @param dir
	 *            direction of the attraction force acting on the node is added
	 ...
	 *            gravitating node
	 * @param dir
	 *            direction of the gravitation force acting on the node is added
	 *            to this variable (output parameter)
	...
	private double addGravitationDir(final Node node, final double[] dir) {
		...
		double tmp = gravFactor * repuFactor * node.weight
				* Math.pow(dist, attrExponent - 2);
		for (int d = 0; d < nrDims; d++) {
	 ...
	 *            repulsing octtree
	 * @param dir
	 *            direction of the repulsion force acting on the node is added
	 *            to this variable (output parameter)
	 * @return approximate second derivation of the repulsion energy
	 */
	private double addRepulsionDir(final Node node, final OctTree tree,
			final double[] dir) {
	...
	private OctTree buildOctTree() {
		...
		double[] minPos = { Double.MAX_VALUE, Double.MAX_VALUE,
				Double.MAX_VALUE };
	...
	private double getAttractionEnergy(final Node node) {
		...
		for (Edge edge : attrEdges.get(node)) {
			...
			if (attrExponent == 0.0) {
				...
				energy += edge.weight * Math.pow(dist, attrExponent)
						/ attrExponent;
			}
		}
		return energy;
	 ...
	 *            parameter)
	 */
	private void getDirection(final Node node, final OctTree octTree,
			final double[] dir) {
		for (int d = 0; d < nrDims; d++)
			dir[d] = 0.0;
	...
	private double getEnergy(final Node node, final OctTree octTree) {
		return getRepulsionEnergy(node, octTree) + getAttractionEnergy(node)
				+ getGravitationEnergy(node);
	}
	...
	private double getGravitationEnergy(final Node node) {
		...
		if (attrExponent == 0.0) {
			...
			return gravFactor * node.weight * Math.pow(dist, attrExponent)
					/ attrExponent;
		}
	}
	...
	private void initEnergyFactors() {
		...
		if (repuSum > 0.0 && attrSum > 0.0) {
			...
			gravFactor = density * repuSum
					* Math.pow(gravFactor, attrExponent - repuExponent);
		} else {
			repuFactor = 1.0;
		}
	}
	 ...
	 * 
	 * @param positions
	 *            position in 3D space for each node. Is not copied and serves
	 *            as input and output parameter. Each position must be a
	 *            <code>double[3]</code>. If the input is two-dimensional (i.e.
	 *            the third array element is 0.0 for all nodes), the output is
	 *            also two-dimensional. Different nodes with nonzero weights
	 *            must have different positions. Random initial positions are
	 *            appropriate.
	 * @param nrIterations
	 *            number of iterations. Choose appropriate values by observing
	 *            the convergence of energy. A typical value is 100.
	 */
	public void minimizeEnergy(final Map<Node, double[]> positions,
			final int nrIterations) {
		if (nodes.size() <= 1)
			return;
		...
		status = "initial energy " + energySum;
		...
		for (int step = 1; step <= nrIterations; step++) {
			...
			status = "iteration " + step + "   energy " + energySum
					+ "   repulsion " + repuExponent;
		}
		printStatistics(octTree);
		if (octTree.getHeight() >= OctTree.MAX_DEPTH) {
			System.err
					.println("The node distances in the layout are extremely nonuniform.\n"
	...
	private void printStatistics(final OctTree octTree) {
		...
		meanAttrEnergy = (attrExponent == 0.0) ? Math.exp(meanAttrEnergy
				/ attrSum) : Math.pow(meanAttrEnergy * attrExponent / attrSum,
				1.0 / attrExponent);
		System.out.println("Weighted mean of attraction energy: "
				+ meanAttrEnergy);
		...
		meanRepuEnergy = (repuExponent == 0.0) ? Math.exp(-meanRepuEnergy
				/ repuSum) : Math.pow(-meanRepuEnergy * repuExponent / repuSum,
				1.0 / repuExponent);
		System.out.println("Weighted mean of repulsion energy: "
				+ meanRepuEnergy);

		System.out.println("Mean attraction / mean repulsion: "
				+ meanAttrEnergy / meanRepuEnergy);
	}
