 ...
 * LinLog energy model, see the (freely downloadable) article Andreas Noack: <a
...
public class MinimizerClassic {
	 ...
	 *            number of coordinates of each node.
	 */
	public MinimizerClassic(final Collection<Node> nodes,
			final Collection<Edge> attrEdges, final double repuExponent,
			final double attrExponent, final double gravFactor, final int nrDims) {
	...
	private double getEnergy(final Node node) {
		return getRepulsionEnergy(node) + getAttractionEnergy(node)
	 ...
	 * 
	 * @param positions
	 *            position in <code>nrDims</code>-dimensional space for each
	 *            node. Is not copied and serves as input and output parameter.
	 *            Each position must be a <code>double[nrDims]</code>. If the
	 *            input is two-dimensional (i.e. the third array element is 0.0
	 *            for all nodes), the output is also two-dimensional. Different
	 *            nodes with nonzero weights must have different positions.
