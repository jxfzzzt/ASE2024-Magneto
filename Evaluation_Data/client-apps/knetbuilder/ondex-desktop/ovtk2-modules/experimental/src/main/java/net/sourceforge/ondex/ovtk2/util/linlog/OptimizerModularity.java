...
public class OptimizerModularity {
	 ...
	 * @param atpairs
	 *            total weighted node pairs of the graph
	 * @return clustering with large Modularity, as map from graph nodes to
	 *         cluster IDs.
	 */
	private Map<Node, Integer> cluster(final Collection<Node> nodes,
			final List<Edge> edges, final double atedges, final double atpairs) {
		status = "Contracting " + nodes.size() + " nodes, " + edges.size()
				+ " edges";
		...
		for (Edge edge : edges) {
			...
			if (edge.startNode.equals(edge.endNode))
				continue;
			if (nodeToContr.containsKey(edge.startNode)
					|| nodeToContr.containsKey(edge.endNode))
				continue;
			// randomize contraction
			// if (!nodeToContr.isEmpty() && Math.random() < 0.5) continue;

			status = " Contracting " + edge;
			// edge.startNode.name + " " + edge.endNode.name
			Node contrNode = new Node(edge.startNode.concept,
		...
		for (Node node : nodes) {
			if (!nodeToContr.containsKey(node)) {
				Node contrNode = new Node(node.concept, node.weight);
		...
		for (Node contrStart : startToEndToWeight.keySet()) {
			...
			for (Node contrEnd : endToWeight.keySet()) {
				Edge contrEdge = new Edge(contrStart, contrEnd,
						endToWeight.get(contrEnd));
				contrEdges.add(contrEdge);
			}
		}

		// cluster contracted graph
		Map<Node, Integer> contrNodeToCluster = cluster(contrNodes, contrEdges,
				atedges, atpairs);

		// decontract clustering
		Map<Node, Integer> nodeToCluster = new HashMap<Node, Integer>();
		for (Node node : nodeToContr.keySet()) {
			nodeToCluster.put(node,
					contrNodeToCluster.get(nodeToContr.get(node)));
		}
		...
		for (Edge edge : edges)
			nodeToEdge.get(edge.startNode).add(edge);
	 ...
	 * 
	 * @param nodes
	 *            weighted nodes of the graph. It is recommended to set the
	 *            weight of each node to the sum of the weights of its edges.
	 *            Weights must not be negative.
	 * @param edges
	 *            weighted edges of the graph. Omit edges with weight 0.0 (i.e.
	 *            non-edges). For unweighted graphs use weight 1.0 for all
	 *            edges. Weights must not be negative. Weights must be
	 *            symmetric, i.e. the weight from node <code>n1</code> to node
	 *            <code>n2</code> must be equal to the weight from node
	 *            <code>n2</code> to node <code>n1</code>.
	 * @param ignoreLoops
	 *            set to <code>true</code> to use an adapted version of
	 *            Modularity for graphs without loops (edges whose start node
	 *            equals the end node)
	 * @return clustering with large Modularity, as map from graph nodes to
	 *         cluster IDs.
	 */
	public Map<Node, Integer> execute(final List<Node> nodes,
			final List<Edge> edges, final boolean ignoreLoops) {
	 ...
	 * 
	 * @param interAtedges
	 *            edge weight between different clusters
	 * @param interAtpairs
	 *            weighted node pairs between different clusters
	 * @param atedges
	 *            total edge weight of the graph
	 * @param atpairs
	 *            total weighted node pairs of the graph
	 ...
	 */
	private double quality(final double interAtedges,
			final double interAtpairs, final double atedges,
			final double atpairs) {
	 ...
	 * 
	 * @param nodeToCluster
	 *            graph nodes with their current clusters (input and output
	 *            parameter)
	 * @param nodeToEdges
	 *            graph nodes with their incident edges
	 * @param atedges
	 *            total edge weight of the graph
	 * @param atpairs
	 *            total weighted node pairs of the graph
	 */
	private void refine(final Map<Node, Integer> nodeToCluster,
			final Map<Node, List<Edge>> nodeToEdges, final double atedges,
			final double atpairs) {
		if (cancelled)
			return;
		...
		for (List<Edge> edges : nodeToEdges.values()) {
			for (Edge edge : edges) {
				if (!nodeToCluster.get(edge.startNode).equals(
						nodeToCluster.get(edge.endNode))) {
		...
		status = "Refining " + nodeToCluster.keySet().size()
		...
		while (quality < prevQuality) {
			...
			for (Node node : nodeToCluster.keySet()) {
				...
				double[] clusterToAtedges = new double[nodeToCluster.keySet()
						.size() + 1];
				...
				for (int newCluster = 0; newCluster <= maxCluster + 1; newCluster++) {
					...
					double newInterPairs = interAtpairs
							+ clusterToAtnodes[cluster]
							* clusterToAtnodes[cluster]
							- (clusterToAtnodes[cluster] - node.weight)
							* (clusterToAtnodes[cluster] - node.weight)
							+ clusterToAtnodes[newCluster]
							* clusterToAtnodes[newCluster]
							- (clusterToAtnodes[newCluster] + node.weight)
							* (clusterToAtnodes[newCluster] + node.weight);
					double newInterEdges = interAtedges
							+ clusterToAtedges[cluster]
							- clusterToAtedges[newCluster];
					double newQuality = quality(newInterEdges, newInterPairs,
							atedges, atpairs);
				...
				if (bestQuality < quality) {
					...
					status = " Moving " + node + " to " + bestCluster + ", "
