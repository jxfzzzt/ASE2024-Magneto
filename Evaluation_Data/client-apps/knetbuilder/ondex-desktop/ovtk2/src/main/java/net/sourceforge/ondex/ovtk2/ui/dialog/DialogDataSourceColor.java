...
public class DialogDataSourceColor extends OVTK2Dialog {
	...
	public DialogDataSourceColor(String selection) {
		...
		this.getContentPane().add(makeButtonsPanel("Dialog.CvColor.Apply", "Dialog.CvColor.Cancel"), BorderLayout.SOUTH);
		this.pack();
	}
	...
	private JPanel makeProperties(String selection) {
		...
		TitledBorder propertiesBorder = BorderFactory.createTitledBorder(Config.language.getProperty("Dialog.CvColor.Cv"));
		...
		// setup table
		model = new ColorTableModel(colors, Config.language.getProperty("Dialog.CvColor.TableName"), Config.language.getProperty("Dialog.CvColor.TableColor"));
		...
		int width = Config.language.getProperty("Dialog.CvColor.TableColor").length() * 7;
