...
import net.sourceforge.ondex.parser.aracyc.objects.Pathway;
/**
 * Transforms a net.sourceforge.ondex.parser.aracyc2.sink.Pathway to a Concept.
 * @author peschr
 */
public class PathwayTransformer extends AbstractTransformer {
	...
	public void nodeToConcept(AbstractNode node) {
		...
		ONDEXConcept concept = graph.getFactory().createConcept(pathway.getUniqueId(), dataSourceAraC,
				ccPathway, etIMPD);
	...
	public void pointerToRelation(AbstractNode node) {
		...
		while(i.hasNext()){
			...
			//pathwayNode.getConcept().addContext(node.getConcept());
