 ...
 */
public class DistributedMemoryONDEXGraph extends AbstractONDEXGraph {
    //Hazelcast instance for this machine
	...
	private final Map<Integer, ONDEXRelation> idToRelation;

	// contains all concepts indexed by id
	private final Map<Integer, ONDEXConcept> idToConcept;
	 ...
	 *            name of graph
	 */
	public DistributedMemoryONDEXGraph(HazelcastInstance hz, String name) {
		this(hz, name, null);
	}
	 ...
	 *            ONDEXListener
	 */
	public DistributedMemoryONDEXGraph(HazelcastInstance hz, String name, ONDEXListener l) {
		super(name, new DistributedMemoryONDEXGraphMetaData(hz, name));
		if (l != null)
			ONDEXEventHandler.getEventHandlerForSID(getSID())
					.addONDEXONDEXListener(l);
        this.hz = hz;
		this.keyToRelation = hz.getMap(name+".keyToRelation");    //new HashMap<RelationKey, ONDEXRelation>();
		this.idToRelation = hz.getMap(name+".idToRelation"); //new DualHashBidiMap<Integer, ONDEXRelation>();
		this.idToConcept = hz.getMap(name+".idToConcept"); //new DualHashBidiMap<Integer, ONDEXConcept>();
		this.dataSourceToConcepts = hz.getMap(name+".dataSourceToConcepts"); //new HashMap<DataSource, Set<ONDEXConcept>>();
		this.conceptClassToConcepts = hz.getMap(name+".conceptClassToConcepts"); //new HashMap<ConceptClass, Set<ONDEXConcept>>();
		this.attributeNameToConcepts = hz.getMap(name+".attributeNameToConcepts"); //new HashMap<AttributeName, Set<ONDEXConcept>>();
		this.evidenceTypeToConcepts = hz.getMap(name+".evidenceTypeToConcepts"); //new HashMap<EvidenceType, Set<ONDEXConcept>>();
		this.conceptToRelations = hz.getMap(name+".conceptToRelations"); //new HashMap<ONDEXConcept, Set<ONDEXRelation>>();
		this.dataSourceToRelations = hz.getMap(name+".dataSourceToRelations"); //new HashMap<DataSource, Set<ONDEXRelation>>();
		this.conceptClassToRelations = hz.getMap(name+".conceptClassToRelations"); // new HashMap<ConceptClass, Set<ONDEXRelation>>();
		this.relationTypeToRelations = hz.getMap(name+".relationTypeToRelations"); // new HashMap<RelationType, Set<ONDEXRelation>>();
		this.attributeNameToRelations = hz.getMap(name+".attributeNameToRelations"); // new HashMap<AttributeName, Set<ONDEXRelation>>();
		this.evidenceTypeToRelations = hz.getMap(name+".evidenceTypeToRelations"); // new HashMap<EvidenceType, Set<ONDEXRelation>>();
		this.tagToConcepts = hz.getMap(name+".tagToConcepts"); // new HashMap<ONDEXConcept, Set<ONDEXConcept>>();
		this.tagToRelations = hz.getMap(name+".tagToRelations"); // new HashMap<ONDEXConcept, Set<ONDEXRelation>>();
		this.conceptToTags = hz.getMap(name+".conceptToTags"); // new HashMap<ONDEXConcept, Set<ONDEXConcept>>();
		this.relationToTags = hz.getMap(name+".relationToTags"); // new HashMap<ONDEXRelation, Set<ONDEXConcept>>();
		this.conceptToEvidence = hz.getMap(name+".conceptToEvidence"); // new HashMap<ONDEXConcept, Set<EvidenceType>>();
		this.relationToEvidence = hz.getMap(name+".relationToEvidence"); // new HashMap<ONDEXRelation, Set<EvidenceType>>();
	}
	...
	protected Set<ONDEXConcept> retrieveConceptAll() {
		...
		return new HashSet<ONDEXConcept>(idToConcept.values());
	}
	...
	protected Set<ONDEXRelation> retrieveRelationAll() {
        //FIXME - this is wasteful...
		return new HashSet<ONDEXRelation>(idToRelation.values());
	}
