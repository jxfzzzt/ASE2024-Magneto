...
public class FileRegistry {
	...
	/**
	 * @param baseDir
	 *            the reference dir to look for resources
	 */
	private FileRegistry(String baseDir) {
		this.baseDir = baseDir;
		ExecutorService EXECUTOR = Executors
				.newFixedThreadPool(resources.length);
		...
		for (final DataResource resource : resources) {
			...
			futures.add(EXECUTOR.submit(new Callable<DataResource>() {
	...
	public enum DataResource {
		GENES(GENES_ZIP, GENES_FILE, GENES_DIR), KGML(KGML_ZIP, KGML_FILE,
				KGML_DIR), LIGAND(LIGAND_ZIP, LIGAND_FILE, LIGAND_DIR), PATHWAY(
				PATHWAY_ZIP, PATHWAY_FILE, PATHWAY_DIR), BRITE(BRITE_ZIP,
				BRITE_FILE, BRITE_DIR), MEDICUS(MEDICUS_ZIP, MEDICUS_FILE,
				MEDICUS_DIR);
		...
		/**
		 * @param tarArchive
		 *            a tar.gz compressed file archive
		 * @param dir
		 *            a uncompressed dir archive
		 * @param zipArchive
		 *            a zip compressed file archive
		 */
	...
	private static DataResource[] resources = new DataResource[] {
			DataResource.GENES, DataResource.KGML, DataResource.LIGAND,
			DataResource.PATHWAY, DataResource.BRITE, DataResource.MEDICUS };
	...
	public FileIndex getIndex(DataResource resource) throws IOException {
		...
		if (index == null) {
			if (new File(baseDir + File.separator + resource.getDir()).exists()) {
				index = new FileIndex(new File(baseDir + File.separator
						+ resource.getDir()));
			} else if (new File(baseDir + File.separator
					+ resource.getZipArchive()).exists()) {
				index = new FileIndex(new File(baseDir + File.separator
						+ resource.getZipArchive()));
			} else if (new File(baseDir + File.separator
					+ resource.getTarArchive()).exists()) {
				index = new FileIndex(new File(baseDir + File.separator
						+ resource.getTarArchive()));
			} else {
				throw new IOException(resource + " does not exist at "
						+ baseDir + " in the form of " + resource.getDir()
						+ ", " + resource.getZipArchive() + ", or "
						+ resource.getTarArchive());
			}
	...
	public static void main(String[] args) {
		// try {
		String pathToKegg = "D:/Data/importdata/kegg56";

		FileRegistry.getInstance(pathToKegg);
	}
}
