...
MIMICRY_RESULT_DIR = os.path.dirname(__file__)
...
def validate_mimicry_result():
    print('mimicry evaluate result:', MIMICRY_RESULT_DIR)
    ...
    for project_name in client_projects_path:
        ...
        for path in Path(our_result_project_dir).iterdir():
            if path.is_dir() and path.name not in BAN_DIR_NAME:
                vul_name = path.name
                mimicry_result_dir = os.path.join(MIMICRY_RESULT_DIR, vul_name, project_name)
    ...
    print('mimicry result validate pass!')


def main():
    validate_mimicry_result()
